...
export default function MemberManagement({ member, loading, MemberRequest }) {
    return (
        <div className='flex flex-1 items-start bg-gray-700 '>
            <Loginchairmanheader />

            {loading ? (
                <img className='m-[auto]' src='/Assets/loading.gif'></img>
            ) : (
                <div className='w-full font-bold font-2xl'>
                    <p className='text-white text-center text-2xl m-[auto]'>Member Management</p>
                    <div className='flex  gap-4 p-5 flex-wrap justify-between text-white font-semibold'>
                        {member.map((data, index) => (
                            <div key={index} className='p-5 rounded bg-gray-500 shadow w-[300px]'>
