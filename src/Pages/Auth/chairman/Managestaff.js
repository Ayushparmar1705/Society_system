...
export default function MemberManagement({ result, loading, inActivestaff }) {
    return (
                    ...
                    <div className='flex  gap-4 p-5 flex-wrap justify-between font-semibold'>
                        {result.map((data, index) => (
                            <div key={index} className='p-5 rounded bg-gray-50 shadow w-[300px]'>
                                ...
                                {data.is_active === 0 ? (
                                    <button className='bg-blue-500 hover:bg-blue-400 text-white p-2 w-full rounded'>Active</button>
                                ) : (
